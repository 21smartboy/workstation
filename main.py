...
# 将数据分隔为train_set和test_set
x_train, x_test, y_train,y_test = train_test_split(x,y, test_size=0.25, random_state=42)
# x_test=torch.tensor(x_test.values.tolist())
# x_test=DataLoader(x_test,shuffle=False,batch_size=128)
#1  138937
...
class part1_data(Dataset):
    def __init__(self,data,label):
        ...
        # self.label=label.tolist()        #将series类型转化为list
...
class my_modle(nn.Module):
    def __init__(self):
        ...
        self.leaner3=nn.Linear(16, 14)
    def forward(self,x):
        x=x.to(torch.float)
        x=x.view(-1,46)
        x=F.relu(self.leaner1(x))
        x=F.relu(self.leaner2(x))
        return self.leaner3(x)
model=my_modle()
